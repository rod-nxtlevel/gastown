...
// 2. Respawns the pane using the command from a restart script file
// 3. Re-enables remain-on-exit (respawn-pane resets it to off!)
//
// The startupCmd is persisted to a restart script under townRoot/.runtime/respawn/.
// This ensures respawn always uses the CURRENT agent config, not a stale command
// that was baked into the tmux pane at original session creation time.
// See: stale Codex respawn bug — when default agent changes from codex to claude,
// the old respawn-pane (without explicit command) would replay the original codex command.
//
// Requires remain-on-exit to be set first (called automatically by this function).
func (t *Tmux) SetAutoRespawnHook(session, townRoot, startupCmd string) error {
	if err := validateSessionName(session); err != nil {
		return err
	}
	...
	// Write the startup command to a restart script so respawn uses current config.
	// This file is updated on every start, so agent changes take effect on next respawn.
	respawnDir := filepath.Join(townRoot, ".runtime", "respawn")
	if err := os.MkdirAll(respawnDir, 0755); err != nil {
		return fmt.Errorf("creating respawn directory: %w", err)
	}
	scriptPath := filepath.Join(respawnDir, session+".sh")
	scriptContent := "#!/bin/sh\n# Auto-generated by gt — do not edit.\n# This file is used by the tmux pane-died hook to respawn with current agent config.\nexec " + startupCmd + "\n"
	if err := os.WriteFile(scriptPath, []byte(scriptContent), 0755); err != nil {
		return fmt.Errorf("writing respawn script: %w", err)
	}

	// Sanitize paths for shell safety
	safeSession := strings.ReplaceAll(session, "'", "'\\''")
	safeScript := strings.ReplaceAll(scriptPath, "'", "'\\''")

	// Hook command: wait, respawn with script, then re-enable remain-on-exit
	...
	// The sleep prevents rapid respawn loops if the agent crashes immediately.
	// Using the script file ensures we always use the current agent config.
	hookCmd := fmt.Sprintf(`run-shell "sleep 3 && tmux respawn-pane -k -t '%s' '%s' && tmux set-option -t '%s' remain-on-exit on"`, safeSession, safeScript, safeSession)
